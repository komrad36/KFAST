...
int main(int argc, char* argv[]) {
	...
	std::vector<MyKeypoint> my_keypoints;
	// warmup
	for (int i = 0; i < warmups; ++i) KFAST<nonmax_suppression>(image.data, image.cols, image.rows, my_keypoints, thresh);
	my_keypoints.clear();

	{
		high_resolution_clock::time_point start = high_resolution_clock::now();
		for (int32_t i = 0; i < Kruns; ++i) {
			KFAST<nonmax_suppression>(image.data, image.cols, image.rows, my_keypoints, thresh);
		}
	...
	if (display_images) {
		...
		cv::namedWindow("My FAST", CV_WINDOW_NORMAL | CV_WINDOW_KEEPRATIO);
		cv::imshow("My FAST", image3);
		cv::waitKey(0);
	}
}
